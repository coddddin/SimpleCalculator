 ...
 **C 버튼 누른 후 화면▼**
 ...
 # (과제4) - 부호반전 및 소수점 버튼 기능 추가 구현
 
  ## -과제 내용-  

    • 목표
    § 계산기에 있는 부호반전/소수점 기능 구현
     내용
    
    1. 부호반전 및 소수점 버튼 이벤트 추가 및 연결
    ▶ 피연산자 입력후 부호반전 버튼 클릭 시 해당 피연산자의 부호가 반전됨
    ▶ 피연산자 입력후 소수점 버튼 클릭 시 해당 피연산자에 소수점이 추가됨

    2. 괄호 버튼 추가 및 로직 설계 
    - 괄호 버튼 이벤트 추가 및 연결
    - 괄호 버튼 클릭 시 괄호가 입력창에 추가되고, 계산 로직에서 괄호의 우선순위를 고려하여 계산이 수행됨
    - 예를 들어, 2 + 3 * 4 입력 시, 곱셈이 먼저 수행되어 결과가 14가 됨
 

    3. 연산자 우선순위 구현
    - 계산 로직에서 연산자 우선순위를 고려하여 계산이 수행됨
    - 예를 들어, 2 + 3 * 4 입력 시, 곱셈이 먼저 수행되어 결과가 14가 됨
    - 괄호를 이용하여 우선순위를 변경할 수 있음 (예: (2 + 3) * 4 입력 시, 덧셈이 먼저 수행되어 결과가 20이 됨)
    - 잘못 입력된 괄호에 대한 예외 처리 (예: 열린 괄호가 닫히지 않거나, 닫힌 괄호가 열리지 않은 경우 등)
  
  ## **- 과제(4) 결과화면-**
  
  ** 부호반전 버튼 누르기 전 그리고 소수점 눌렀을 때 화면▼**
  ...
  ** 우선순위 연산자 구현 화면▼**
  ...
  ![과제4 실행화면](img/4-3.jpg)

  ** 괄호 입력 및 우선순위 연산자 구현 화면 (문제의 문제)▼**

  ![과제4 실행화면](img/4-4.jpg)



  # 과제를 마치며..

   -- 무엇인가 시간에 쫒기어 코파일럿을 쓰는 느낌이라 아쉬웠다. 
   
   - 코파일럿의 로직을 쓰는 것을 보고 내가 고치면서 많은 공부가 되었다.

   - 예전에 C++로 구현했던 계산기와 비교해보며 C#에서 Windows Forms의 GUI 개발이 더욱더 편리해졌다는 것을 느낄 수 있었다.
     
   -  못한 부분이 많지만, 그래도 C# 프로그래밍과 Windows Forms 애플리케이션 개발에 대한 이해가 많이 향상된 것 같다.
    
   - 앞으로도 계속해서 C# 프로그래밍을 연습하고, 더 복잡한 애플리케이션을 개발해보고 싶다.